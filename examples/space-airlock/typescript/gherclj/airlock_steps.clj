(ns gherclj.airlock-steps
  "Step routing for the TypeScript implementation of SpaceAirlock.
   Each entry maps a Gherkin phrase to a method call on subject."
  (:require [gherclj.core :refer [defgiven defwhen defthen helper!]]
            [gherclj.frameworks.typescript.node-test :as typescript]))

(helper! "src/space_airlock_steps")

(typescript/describe-setup! "let subject: space_airlock_steps.SpaceAirlockSteps")
(typescript/describe-setup! "beforeEach(() => {\n  subject = new space_airlock_steps.SpaceAirlockSteps()\n})")
