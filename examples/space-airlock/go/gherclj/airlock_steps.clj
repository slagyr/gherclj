(ns gherclj.airlock-steps
  "Step routing for the Go implementation of SpaceAirlock.
   Each entry maps a Gherkin phrase to a method call on `subject`.
   The subject is created per-scenario via gotest/scenario-setup!."
  (:require [gherclj.core :refer [defgiven defwhen defthen helper!]]
            [gherclj.frameworks.go.testing :as gotest]))

;; Import the production package. The string \"alias path\" form gives
;; us an explicit alias the generated tests will use.
